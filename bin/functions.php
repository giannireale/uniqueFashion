...
class Functions {
	...
	public function verifica_login($email_o_nome_utente, $password) {
		$password = sha1($password);
		$query = mysql_query("SELECT email FROM user WHERE (email = '$email_o_nome_utente' OR username='$email_o_nome_utente') AND password = '$password'") or die('Errore: ' . mysql_error());
		$conta = mysql_num_rows($query);
		if ($conta == 1) {
			$risultato = mysql_fetch_object($query);
			$_SESSION['logged'] = 1;
